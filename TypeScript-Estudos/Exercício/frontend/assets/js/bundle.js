...
function checkEmail(input) {
    if (!validator_lib_isEmail__WEBPACK_IMPORTED_MODULE_0___default()(input.value))
        showErrorMessage(input, 'Email não é valido');
}
...
function checkInputs(form) {
    var send = true;
    form.querySelectorAll("." + SHOW_ERROR_MESSAGES).forEach(function (val) {
        send = false;
    });
    return send;
}
